...
| `0x41` | `CFG_SET` | `key(1)`, `value…` | — | set a config key (validated + persisted by the device). See *WiFi provisioning*. Key `0x14` (DATA kind, rw) switches the bridged medium (uart ↔ i2c) on devices that support it. Key `0x15` (WiFi scan): `CFG_SET 0x15` (any value) kicks an async scan; `CFG_GET 0x15` returns `count(1)` then per AP `rssi(1, signed)` · `channel(1)` · `ssid_len(1)` · `ssid…` — empty while scanning or if unsupported. |
